...
import { X, ExternalLink, MapPin, Clock, DollarSign, Briefcase, Link as LinkIcon, Mail, CalendarClock, Bookmark, Copy, Flag } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import ReportModal from './ReportModal'; // นำเข้า Component Modal
...
export default function DetailDrawer({ internship, config, onClose, status, updateTrackStatus, showToast }: DetailDrawerProps) {
  // เพิ่ม State สำหรับควบคุมเปิด-ปิด Modal แจ้งรายงาน
  ...
  return (
    <>
        ...
        >
          ...
          >
            ...
            <div className="sticky bottom-0 bg-zinc-950/80 backdrop-blur-md border-t border-zinc-800 p-4 flex flex-col gap-2">
              {/* ปุ่มสำหรับแจ้งข้อมูลผิดพลาด */}
              <button
                onClick={() => setIsReportModalOpen(true)}
                className="w-full bg-red-500/10 border border-red-500/30 hover:bg-red-500/20 text-red-400 font-semibold py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-all text-xs mb-2"
              >
                <Flag className="w-3.5 h-3.5" /> รายงานข้อมูลบริษัทผิด / ปิดรับแล้ว
              </button>
      ...
      {/* เพิ่ม ReportModal เข้ามาไว้ในระดับเดียวกับ Drawer */}
      <ReportModal 
